...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + (style.FramePadding.x+5.f) * 2.0f, label_size.y + (style.FramePadding.y+5.f) * 2.0f);

    const ImRect bb(pos, pos + size);
    ItemSize(size, style.FramePadding.y+5.f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    ImGui::PushFontShadow(IM_COL32_BLACK);
    RenderTextClipped(bb.Min + (style.FramePadding+ImVec2(5, 5)), bb.Max - (style.FramePadding+ImVec2(5, 5)), label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopFontShadow();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 color = hovered ? GetColorU32(ImGuiCol_FrameBg) : IM_COL32(24, 24, 24, 255);
    window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_FrameBg), color, color);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (*v)
        window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, check_col, check_col, color, color);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = 125;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 35));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, 15), window->DC.CursorPos + ImVec2(size, 35));
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(33 / 255.0f, 33 / 255.0f, 33 / 255.0f, 1.f), 0.f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(0.f, 0.f, 0.f, 1.f), 0.f);

    if (popup_open)
        window->DrawList->AddText(rect.Min, GetColorU32(ImGuiCol_Text, 1.f), label);
    else
        window->DrawList->AddText(rect.Min, GetColorU32(ImGuiCol_TextDisabled, 1.f), label);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * 1.f));
    }
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 pos = FindBestWindowPosForPopupEx(rect.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, rect, ImGuiPopupPositionPolicy_ComboBox);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 4), total_bb.Max);
    ...
    window->DrawList->AddRectFilledMultiColor(frame_bb.Min+ImVec2(1, 1), ImVec2(grab_bb.Max.x, frame_bb.Max.y-1), IM_COL32(24, 24, 24, 255), CMenu::Get().AccentColor.c(), CMenu::Get().AccentColor.c(), IM_COL32(24, 24, 24, 255));

    ImGui::PushFontShadow(IM_COL32(0, 0, 0, 255));
    if (clicked)
    {
        window->DrawList->AddText(g.Font, g.FontSize, total_bb.Min, hovered ? GetColorU32(ImGuiCol_Text) : GetColorU32(ImGuiCol_TextDisabled), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, 4.f, IM_COL32(24, 24, 24, 255));
    draw_list->AddCircle(sv_cursor_pos, 4.f, IM_COL32_BLACK);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (selected)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y+2), ImVec2(bb.Min.x + 1, bb.Max.y-2), CMenu::Get().AccentColor.c());
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y+2), ImVec2(bb.Min.x + 2, bb.Max.y-2), CMenu::Get().AccentColor.c());
        //RenderFrame(bb.Min, bb.Max, col, false, 0.0f);
        //RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
    }
    else if (hovered)
    {
        Color h_col = CMenu::Get().AccentColor;
        h_col.a = 100 / 255.f;
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y + 2), ImVec2(bb.Min.x + 1, bb.Max.y - 2), h_col.c());
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y + 2), ImVec2(bb.Min.x + 2, bb.Max.y - 2), h_col.c());
    }
    ...
    ImGui::PushFontShadow(IM_COL32_BLACK);
    if (!selected) PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    if (label_size.x > 0.0f)
    {
        ImVec2 label_pos = ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y);
        RenderText(label_pos, label);
